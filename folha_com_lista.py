...
def folha_pagamento(funcionarios):
    ...
    if matricula in funcionarios:
        ...
        print('-' * 40)
        print(f"Matrícula: {funcionario[0]}")
        print(f"Nome: {funcionario[1]}")
        print(f"Código da Função: {funcionario[2]}")
        print(f"Salário Bruto: {funcionario[4]}")
        print(f"Percentual de Imposto: {funcionario[6]}%")
...
def relatorio(funcionarios):
    ...
    for matricula in funcionarios:
        funcionario = funcionarios[matricula]
        salario_liquido = calcular_imposto(funcionario)
        print(f"{funcionario[0]:<10} | {funcionario[1]:<20} | {funcionario[2]:<16} | {funcionario[4]:<14.2f} | {salario_liquido:<15.2f}")
...
def funcionario_maior_salario(funcionarios):
    ...
    if funcionario_maior_salario_liquido is not None:
...
def funcionario_mais_faltas(funcionarios):
    ...
    for matricula in funcionarios.keys():
        funcionario = funcionarios[matricula]
        if funcionario[5] > maior_numero_faltas:
            funcionario_maior_numero_faltas = funcionario
            maior_numero_faltas = funcionario[5]
    
    if funcionario_maior_numero_faltas is not None:
        desconto_faltas = funcionario_maior_numero_faltas[5] * (funcionario_maior_numero_faltas[4] / 30)
        print("Funcionário com mais faltas")
        print('-' * 40)
        print(f"Matrícula: {funcionario_maior_numero_faltas[0]}")
        print(f"Nome: {funcionario_maior_numero_faltas[1]}")
        print(f"Código da Função: {funcionario_maior_numero_faltas[2]}")
        print(f"Número de Faltas: {funcionario_maior_numero_faltas[5]}")
        print(f"Desconto no Salário: {desconto_faltas:.2f}")

def calcular_imposto(funcionario):
    imposto = funcionario[4] * (funcionario[6] / 100)
    salario_liquido = funcionario[4] - imposto
...
def imprimir_info_funcionario(funcionario, salario_liquido):
    ...
    print(f"Nome: {funcionario[1]}")
    print(f"Código da Função: {funcionario[2]}")
    print(f"Salário Bruto: {funcionario[4]:.2f}")
    print(f"Percentual de Imposto: {funcionario[6]}%")
    print(f"Salário Líquido: {salario_liquido}")
